var VoxelGrid2 = function (origin, dims, levelSizes, levelSizeIndex) {
	...
	this._voxelIndex = new Uint8Array(size * size * size);
		...
		for (var y = start.y; y <= end.y; y++) {
			...
			for (var x = start.x; x <= end.x; x++) {
				...
				if (obj.intersectBox(bbox)) {
					var vi = this._voxelIndex[c];
					if (!vi) {
						this._voxelIndex[c] = 1;
