var VoxelGrid4 = function (origin, dims, levelSizes, levelSizeIndex, voxelIndex, viOffset, voxels, voOffset, triIndices, triangles) {
	...
	this._voxels = voxels || new Int32Array(this._voxelIndex.buffer, (((1 + size * size * size) * size * size * size) >> 5)*4);
		...
		for (var y = start.y; y <= end.y; y++) {
			...
			for (var x = start.x; x <= end.x; x++) {
				...
				if (obj.intersectBox(bbox)) {
					...
					if (this._leaf === 0) {
						var vx = this._voxels[this._voxelsOffset + c];
						if (vx === 0) {
...
VoxelGrid4.memoryAccess = function(addr, bytes) {
...
VoxelGrid4.prototype.triIntersect = function(index, ray) {
	VoxelGrid4.memoryAccess(1e7 + (index*9*4), 9*4);
	const off = index * 9;
	const v0 = vec3(
		this._triArr[off],
		this._triArr[off + 1],
...
VoxelGrid4.prototype.triListIntersect = function (indexCoord, ray) {
	var minT = Infinity;
	var hit = null;
	Voxel.visitCount++;
	let i = 0;
	for (i = 0; i < 16; i++) {
		VoxelGrid4.memoryAccess(2e7 + ((indexCoord+i)*2), 2);
		let index = this._triIndicesFlat[indexCoord+i];
		if (index === 0) {
			break;
		}
		index--;
		//if (index !== ray.lastTested) {
			ray.light.x += 0.1;
			VoxelGrid.cmpCount++;
...
VoxelGrid4.prototype.intersect = function (ray) {
	...
	VoxelGrid4.memoryAccess(3e7, 8*4);
	...
	while (true) {
        if (c.x < 0 || c.y < 0 || c.z < 0 || c.x >= size || c.y >= size || c.z >= size) {
            ...
            VoxelGrid.stepCount++;
            ray.light.y += 0.02;
			VoxelGrid4.memoryAccess(4*(voxelIndexOffset + (ci >> 5)), 4);
            ...
            if (vi !== 0) {
                ...
                if (onTopLevel) {
					...
					VoxelGrid4.memoryAccess(4*((((1 + size**3) * size**3) >> 5) + voxelsOffset + ci), 4);
					const voxel = this._voxels[voxelsOffset + ci];
					let hit;
					if (voxel & 0x80000000) {
						VoxelGrid4.memoryAccess(1e7 + 36*(voxel & 0x7fff), 9*4);
						VoxelGrid.cmpCount++;
						ray.light.y += 0.125;
						ray.light.z += 0.25;
						hit = this.triIntersect(voxel & 0x7fff, ray);
						if (voxel & 0x40000000) {
							VoxelGrid4.memoryAccess(1e7 + 36*((voxel >> 15) & 0x7fff), 9*4);
							VoxelGrid.cmpCount++;
							ray.light.y += 0.25;
							ray.light.z += 0.25;
						...
						hit = this.triListIntersect(voxel * 16, ray);
					}
