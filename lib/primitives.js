...
Triangle.prototype.intersectCylinder = function(ray, radius) {
	...
	const s0 = sub(ray.o, v0);
	const l0 = length(cross(s0, ray.d));

	const s1 = sub(ray.o, v1);
	const l1 = length(cross(s1, ray.d));

	const s2 = sub(ray.o, v2);
	const l2 = length(cross(s2, ray.d));
