...
Triangle.prototype.intersectBoxFast = function(box) {
	const boxMid = add(box.min, mulS(sub(box.max, box.min), 0.5));
	const pd = sub(boxMid, this._vertices[0]);
...
Triangle.prototype.intersectBox = function(box) {
	if (box.contains(this._vertices[0]) || box.contains(this._vertices[1]) || box.contains(this._vertices[2])) {
		return true;
	}
    ...
    var triangleEdges = [this._e1, this._e2, sub(this._vertices[2], this._vertices[1])];
    for (var i = 0; i < 3; i++)
    for (var j = 0; j < 3; j++)
    {
