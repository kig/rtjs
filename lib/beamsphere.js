...
class Beam {
    ...
    constructor(startX, startY, endX, endY, radius) {
        ...
        this.radius = radius;
        this.triangles = [];
        this.startPoint = fromOct(this.startX, this.startY);
        this.endPoint = fromOct(this.endX, this.endY);
...
class BeamSphere {
    ...
    constructor(origin, beamsPerSide) {
        this.origin = origin;
        this.sphere = new Sphere(origin, 0);
            ...
            for (let x = 0; x < beamsPerSide; x++) {
                for (let ty = 0; ty < beamsPerSide; ty++) {
                    for (let tx = 0; tx < beamsPerSide; tx++) {
                        const beam = new Beam(y/beamsPerSide, x/beamsPerSide, ty/beamsPerSide, tx/beamsPerSide, Math.SQRT2/beamsPerSide);
    ...
    add(obj) {
        this.sphere.radius = max(this.sphere.radius, length(sub(obj._vertices[0], this.origin)));
        this.sphere.radius = max(this.sphere.radius, length(sub(obj._vertices[1], this.origin)));
        this.sphere.radius = max(this.sphere.radius, length(sub(obj._vertices[2], this.origin)));
    ...
    intersect(ray) {
        ...
        const irad = 1/this.sphere.radius;

        let entry = toOct(mulS(sub(add(ray.o, mulS(ray.d, t.entryDistance)), origin), irad));
        let exit = toOct(mulS(sub(add(ray.o, mulS(ray.d, t.exitDistance)), origin), irad));

        let x = floor(entry.x * beamsPerSide) % beamsPerSide;
        if (x < 0) x += beamsPerSide;
        let y = floor(entry.y * beamsPerSide) % beamsPerSide;
        if (y < 0) y += beamsPerSide;
        let tx = floor(exit.x * beamsPerSide) % beamsPerSide;
        if (tx < 0) tx += beamsPerSide;
        let ty = floor(exit.y * beamsPerSide) % beamsPerSide;
        if (ty < 0) ty += beamsPerSide;
