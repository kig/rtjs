...
class Beam {
    ...
    constructor(startX, startY, endX, endY, sphereRadius, radius) {
        this.startX = startX;
        this.startY = startY;
        this.endX = endX;
        this.endY = endY;
        ...
        this.radius = radius;
        this.triangles = [];
        this.startPoint = mulS(fromOct(this.startX, this.startY), this.sphereRadius);
        this.endPoint = mulS(fromOct(this.endX, this.endY), this.sphereRadius);
    ...
    sort() {
        ...
        this.coverage = new Plane(
            add(this.startPoint, mulS(sub(this.endPoint, this.startPoint), 0.5)),
            neg(this.ray.d),
            null
        );
        this.planeUV = orthoBasis(this.ray.d);
        this.planeUV.pop();
        const [u, v] = this.planeUV;
        this.lookup = [];
        const n = 16;
        const n1 = n - 1;
        for (let y = 0; y < n; y++) {
            for (let x = 0; x < n; x++) {
                const uf = ((x/n1) - 0.5) * 2;
                const vf = ((y/n1) - 0.5) * 2;
                const o = diskUVToPoint(this.ray.o, u, v, uf, vf, this.radius);
                const r = new Ray(o, this.ray.d);
                const hit = this.intersect(r);
                this.lookup[y*n + x] = hit;
            }
        }
    }

    fastIntersect(ray, rayOff) {
        const point = add(ray.o, mulS(ray.d, rayOff));
        const n = 16;
        const n1 = n - 1;
        const uv = diskPointToUV(point, this.ray.o, this.planeUV[0], this.planeUV[1], this.radius);
        const x = Math.floor((uv.u / 2 + 0.5) * n - 1e-9);
        const y = Math.floor((uv.v / 2 + 0.5) * n - 1e-9);
        const off = y*n + x;
        if (this.lookup[off]) {
            BeamSphere.primitiveTests++;
            return this.lookup[off].obj.intersect(ray);
        }
        return null;
    }
...
class BeamSphere {
            ...
            for (let x = 0; x < beamsPerSide; x++) {
                for (let ty = 0; ty < beamsPerSide; ty++) {
                    for (let tx = 0; tx < beamsPerSide; tx++) {
                        const beam = new Beam(x/beamsPerSide+d, y/beamsPerSide+d, tx/beamsPerSide+d, ty/beamsPerSide+d, radius, 0.3*1.2*Math.PI*radius/beamsPerSide);
    ...
    sort() {
        this.allBeams.forEach(b => b.sort());
    }
    ...
    intersect(ray) {
        ...
        if (false) {
            ...
            ray.light.y = beam.startY;
            ray.light.z = beam.endY + beam.endX;
            const sd = length(sub(sub(add(ray.o, mulS(ray.d, t.entryDistance)), origin), beam.startPoint));
            const ed = length(sub(sub(add(ray.o, mulS(ray.d, t.exitDistance)), origin), beam.endPoint));
            if (sd < 0.1) {
                ray.light = add(ray.light, vec3(0,1,0));
            } else if (2*(sd / beam.radius) % 1 < 0.1) {
                ray.light = add(ray.light, vec3(0,1,1));
            } else if (rayLineDistance({o: sub(ray.o, origin), d: ray.d}, beam.startPoint, beam.endPoint) < beam.radius*0.05) {
        ...
        return beam.fastIntersect(ray, t.entryDistance);
    }
