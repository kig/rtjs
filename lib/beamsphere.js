/*
    Bounding beam hierarchy

    Match ray to beam.
    Search beam primitives for intersection.
    Primitives are sorted in beam direction, enabling fast exit when next primitive is further away than hit.
*/
...
class Beam {
    ...
    constructor(startX, startY, endX, endY, sphereOrigin, sphereRadius, radius, halfSide) {
        this.startX = startX;
        this.startY = startY;
        this.endX = endX;
        this.endY = endY;
        this.uvX0 = this.endX - halfSide;
        this.uvX1 = this.endX + halfSide;
        this.uvY0 = this.endY - halfSide;
        this.uvY1 = this.endY + halfSide;
        this.sphereRadius = sphereRadius;
        this.origin = sphereOrigin;
        this.radius = radius;
        this.diskSize = 4;
        this.triangles = [];
        this.lookup = [];
    ...
    toOct(point) {
        ...
        var po = sub(point, this.origin);
        var po_l = length(po);
        var t2 = po_l * po_l + radius * radius - 2 * po_l * radius * dot(this.ray.d, divS(po, po_l));
        var t = sqrt(t2);
        var pointOnSphere = add(point, mulS(this.ray.d, t));
        return toOct(sub(pointOnSphere, this.origin));
    }

    insideUV(uv) {
        return uv.x >= this.uvX0 && uv.x <= this.uvX1 && uv.y >= this.uvY0 && uv.y <= this.uvY1;
    }
    ...
    intersectTriangle(tri) {
        return this.insideUV(this.toOct(tri._vertices[0])) || this.insideUV(this.toOct(tri._vertices[1])) || this.insideUV(this.toOct(tri._vertices[2]));
    }
...
class BeamSphere {
                    ...
                    for (let tx = 0; tx < beamsPerSide; tx++) {
                        if (x !== tx || y !== ty) {
                            const beam = new Beam(
                                ...
                                origin, radius, 
                                1.2*Math.PI*radius/beamsPerSide, d
                            );
                            beams[y*beamsPerSide + x][ty*beamsPerSide + tx] = beam;
    ...
    intersect(ray) {
        ...
        let x = floor(entry.x * beamsPerSide - 1e9);
        let y = floor(entry.y * beamsPerSide - 1e9);
        let tx = floor(exit.x * beamsPerSide - 1e9);
        let ty = floor(exit.y * beamsPerSide - 1e9);
