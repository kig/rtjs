...
class Beam {
    ...
    sort() {
        ...
        for (let y = 0; y < n; y++) {
            for (let x = 0; x < n; x++) {
                const uf = ((x/n1) - 0.5) * 2;
                const vf = ((y/n1) - 0.5) * 2;
                const o = diskUVToPoint(this.ray.o, uvw, uf, vf, radius);
    ...
    fastIntersect(ray, rayStart, rayEnd) {
        const point = add(ray.o, mulS(ray.d, rayStart));
        const n = this.diskSize;
        const uv = diskPointToUV(point, this.ray.o, this.planeUVW, this.radius);
...
class BeamGrid {
    constructor(startPoint, endPoint, beamsPerSide) {
