...
BVHNode.prototype.intersect = function(ray, closestHit) {
	if (closestHit === undefined) {
		closestHit = Infinity;
	}
	BVHNode.visitedCount++;
	var t = this._box.intersect(ray);
	var hit = {obj: null, distance: closestHit};
	if (t && t.distance < closestHit) {
		var leaf = this.leaf ? 1 : 0;
		for (var i=0; i<this.children.length; i++) {
			BVHNode.primitiveTests += leaf;
			var tc = this.children[i].intersect(ray, hit.distance);
			if (tc && tc.distance < hit.distance) {
				hit = tc;
			}
		}
	}
