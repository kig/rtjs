...
class SerializedVG {
    ...
    intersect(ray) {
        ...
        if (this._isLeaf) {
            ...
            this.intersectLevel2(ray, this._iarray[0] * 18 + 4, closestHit);

        } else {

            const headOff = this._iarray[0] * 18 + 4;
            const origin = vec3(
                this._array[headOff],
                this._array[headOff + 1],
                this._array[headOff + 2]
            );
            const size = this._iarray[headOff + 3];
            const dims = vec3(
    ...
    intersectLevel2(ray, coff, closestHit) {
        ...
        const childSize = this._iarray[headOff + 7];
        const box = new Box(origin, add(origin, dims));
        const scale = dims.x / size;
        const voxelsOff = headOff + 8;
