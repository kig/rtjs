...
class VoxelGrid3 {
    ...
    constructor(origin, dims, levelSizes, levelSizeIndex) {
        ...
        this._voxels = new Float32Array(size * size * size);
        this._children = [];
    }
    ...
    serialize(root) {
        ...
        if (this._leaf === 0) {
            ...
            header[3] = this._levelSize;
            header[4] = this._dims.x;
            header[5] = this._dims.y;
            header[6] = this._dims.z;
            header[7] = voxelSize;
            let f32 = new Float32Array(buf, headerLength * 4);
            for (let i = 0; i < f32.length; i++) {
                f32[i] = -1;
            }
            f32.set(this._voxels);
            const triArray = new Float32Array(buf, headerLength * 4 + this._voxels.byteLength, this._children.length * voxelSize + 1);
            ...
            header[5] = this._dims.y;
            header[6] = this._dims.z;
            header[7] = this._children.length;
            let f32 = new Float32Array(buf, headerLength * 4);
            f32.set(this._voxels);
            const cArray = new Float32Array(buf, headerLength * 4 + this._voxels.byteLength, this._children.length);
            bufs.push(header);
            let childOffset = 0;
            for (let i = 0; i < this._children.length; i++) {
...
class SerializedVG {
    ...
    intersect(ray) {
        ...
        if (this._isLeaf) {
            ...
            this.intersectLevel2(ray, this._array[0] * 18 + 4, closestHit);
