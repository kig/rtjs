...
var randomVec3Unit = function() {
	const a = random() * (2.0 * Math.PI);
	const r = 2.0 * random() - 1.0;
	const sr = sqrt(1.0 - r * r);
    return vec3(sr * cos(a), sr * sin(a), r);
}

var diskPointInPlace = function(v) {
	const a = random() * (2.0 * Math.PI);
	const r = random();
	const sr = sqrt(r);
	v.x = cos(a) * sr;
...
var diskPoint = function() {
	...
	const r = random();
	const sr = sqrt(r);
	return vec3(cos(a) * sr, sin(a) * sr, 0);
};
...
var diskUVToPoint = function(o, uvw, uf, vf, radius) {
	uf *= radius;
	vf *= radius;
	const uv = add(mulS(uvw[0], uf), mulS(uvw[1], vf));
	return add(o, uv)
};

var diskPointToUV = function(p, o, uvw, radius) {
	const d = sub(p, o);
	const uf = dot(d, uvw[0]) / radius;
	const vf = dot(d, uvw[1]) / radius;
	return {u: uf, v: vf};
};

var vecToArray = function(v) {
	return [v.x, v.y, v.z];
};