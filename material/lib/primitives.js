...
var Triangle = function(vertices, normals, color) {
	...
	this._e3 = sub(this._vertices[2], this._vertices[1]);
	const crossE = cross(this._e1, this._e2);
	const lenCrossE = length(crossE);
	this.area = 0.5 * lenCrossE;
    this._nml = divS(crossE, lenCrossE);
...
Triangle.prototype.intersect = function(ray) {
	...
	if (abs(u) < 0.05 || abs(v) < 0.05 || abs(u+v) > 0.95) {
		//ray.light.y += 0.5;
		//ray.light.z += 0.5;
	}
...
Triangle.prototype.intersectAsPlane = function(ray) {
	var pd = dot(this._nml, ray.d);
	if (abs(pd) > 0.00001) {
		var dist = dot(this._nml, sub(this._vertices[0], ray.o)) / pd;
		return dist > 0 ? {obj: this, distance: dist} : null;
	}
	return null;
};
...
Triangle.setBoxVertices = function(box) {
	let i = -1;

	this.boxVertices[++i].x = box.min.x;
	this.boxVertices[i].y = box.min.y;
	this.boxVertices[i].z = box.min.z;

	this.boxVertices[++i].x = box.max.x;
	this.boxVertices[i].y = box.max.y;
	this.boxVertices[i].z = box.max.z;
...
Triangle.prototype.intersectBoxFast = function(box) {
	const pd = sub(box.mid, this._vertices[0]);
	const boxRadius = 0.8660254037844386 * (box.max.x - box.min.x);
	return abs(dot(pd, this._nml)) <= boxRadius;
};
...
Triangle.prototype.intersectBox = function(box) {
	...
	Triangle.setBoxVertices(box);

	// Test the triangle normal
    var triangleOffset = dot(this._nml, this._vertices[0]);
    this.intersectBoxProject(Triangle.boxVertices, this._nml, minMax);
    if (minMax.max < triangleOffset || minMax.min > triangleOffset) {
		return false; // No intersection possible.
	}

    // Test the nine edge cross-products
    var triangleEdges = [this._e1, this._e2, this._e3];
    for (var i = 0; i < 3; i++)
    for (var j = 0; j < 3; j++)
    {
        // The box normals are the same as it's edge tangents
        var axis = cross(triangleEdges[i], boxNormals[j]);
        this.intersectBoxProject(Triangle.boxVertices, axis, minMax);
...
Box.prototype.intersect = function(r) {
	...
	var tmax = min(t1_.x, min(t1_.y, t1_.z));
