...
function setFocalLength(value) {
	window.focalLength.value = value;
}
...
function CameraControls(camera, canvas) {
	...
	state.startFocalLength = getFocalLength();

	hammer.get('pinch').set({ enable: true });
	hammer.on('pinchstart', function(ev) {
		startFocalLength = getFocalLength();
		state.pinching = true;
	});
	...
	hammer.on('pinch', function(ev) {
		setFocalLength(max(min(startFocalLength * ev.scale, 200), 14));
		state.pinching = true;
		state.changed = true;
		ev.preventDefault();
	});
	...
	canvas.onwheel = function(ev) {
		...
		setFocalLength(max(min(getFocalLength() * f, 200), 14));
		state.changed = true;
		ev.preventDefault();
	};
...
CameraControls.prototype.updateCameraPosition = function() {
	var state = this;
	state.camera.position.set(Math.cos(state.theta)*Math.cos(state.alpha), Math.sin(state.alpha), Math.cos(state.alpha)*Math.sin(state.theta)).normalize().multiplyScalar(7.5);
